...
clean ()
{
    ...
    rm -rf o* .w* build/AIK/*.c* build/AIK/s* build/*.p* build/*er* && git restore arch/arm64/configs/$KERNEL_DEFCONFIG
...
Options:
    -m, --model [value]    Specify the Model Code of the Phone (default: hlte)
...
KERNEL_DEFCONFIG=exynos7870-"$MODEL"_defconfig
case $MODEL in
a3y17lte)
    DEVICE="A3 (2017)"
;;
...
detect_env ()
{
    ...
    DATE=`date +"%Y%m%d"`
    REPO_URL="https://raw.githubusercontent.com/StardustMod/build/refs/heads/exynos7870/"
    export KBUILD_BUILD_USER=lavenly
    ...
    if test -f "/usr/bin/python2.7" && test -f "/usr/bin/python2"; then
        ...
        if [[ $EUID -ne 0 ]]; then
            ...
            sudo apt install -y ./libpython2.7-minimal_2.7.18-13ubuntu1.5_amd64.deb ./libpython2.7-stdlib_2.7.18-13ubuntu1.5_amd64.deb ./python2.7-minimal_2.7.18-13ubuntu1.5_amd64.deb ./python2.7_2.7.18-13ubuntu1.5_amd64.deb
            rm -rf *.deb
    ...
    if test -d "build/AIK"; then
        ...
        git submodule add -b image -f -q https://github.com/StardustMod/build build/AIK > /dev/null && chmod +x build/AIK/mk*
...
toolchain ()
{
    ...
    if test -d "toolchain"; then
        quotes "Linaro GCC 7.5-2019.12 Directory Found!"
    else
        quotes "Add Linaro GCC 7.5-2019.12 as Submodule"
        git submodule add -f -q -b arm64/latest-7 https://github.com/cheemskek/linaro toolchain/gcc-linaro-7.5.0 > /dev/null
        check "Linaro GCC 7.5-2019.12"
    fi

    GCC=$PWD/toolchain/gcc-linaro-7.5.0/bin/
    ARGS="
        ARCH=arm64 O=out \
...
kernelsu ()
{
    ...
    if ! test -d "drivers/kernelsu"; then
        quotes "Add KernelSU as Submodule"
        separator

        if test -d "KernelSU"; then
            rm -rf Ke*
        fi
...
kernel ()
{
    ...
    separator
    noquotes "Device: Galaxy $DEVICE ("$MODEL")"
    noquotes "SOC: Exynos 7870"
    noquotes "Defconfig: $KERNEL_DEFCONFIG"
    noquotes "Kernel Version: $KERNEL_VERSION"
    noquotes "Build Date: `date +"%Y-%m-%d"`"

    if [ -z $KSU ]; then
        noquotes "KernelSU: Not Include"
    else
        noquotes "KernelSU: Include"
    fi

    sed -i "s/CONFIG_LOCALVERSION=\"-SIMPLE-KERNEL_V1.0\"/CONFIG_LOCALVERSION=\"-Stardust-$KERNEL_VERSION-$MODEL\"/" arch/arm64/configs/$KERNEL_DEFCONFIG
...
ramdisk ()
{
    ...
    mv ../../../out/arch/arm64/boot/dtb.img boot.img-dt
    echo -e "0x10000000" > boot.img-base
    echo -e "" > boot.img-board
    echo -e "" > boot.img-cmdline
    echo -e "data" > boot.img-dttype
    ...
    echo -e "19802128" > boot.img-origsize
    echo -e "2020-06" > boot.img-os_patch_level
    ...
    echo -e "SEAndroid" > boot.img-tailtype
    popd > /dev/null
...
build_zip ()
{
    ...
    sed -i "s/ui_print(\" Kernel Device: \");/ui_print(\" Kernel Device: Galaxy $DEVICE ($MODEL)\");/" build/out/$MODEL/zip/META-INF/com/google/android/updater-script
    sed -i "s/ui_print(\" Kernel Toolchain: \");/ui_print(\" Kernel Toolchain: Linaro GCC 7.5-2019.12\");/" build/out/$MODEL/zip/META-INF/com/google/android/updater-script

    if [[ "$LOCAL" == "y" ]] || [[ "$RELEASE" == "y" ]]; then
        sed -i "s/CONFIG_LOCALVERSION=\"-Stardust-$KERNEL_VERSION-$MODEL\"/CONFIG_LOCALVERSION=\"-StardustKernel-$KERNEL_VERSION-$DATE-$DEVICE-$MODEL\"/" arch/arm64/configs/$KERNEL_DEFCONFIG
...
(
    ...
    if [[ "$KSU" == "y" ]]; then
        echo "# CONFIG_KSU_LSM_SECURITY_HOOKS is not set" >> arch/arm64/configs/$KERNEL_DEFCONFIG
        kernelsu
    fi
